...
DEFAULT_OUTPUT_DIR = DATA_ROOT / "processed_dataset" / "lyrics_e5_large"
E5_EMBEDDING_DIM = 1024
...
def encode_batches(
    ...
    for start in range(0, total, batch_size):
        ...
        with torch.inference_mode():
            ...
            pooled = F.normalize(pooled, p=2, dim=1).float()
            projected = projection(pooled)

        e5_embeddings.extend(pooled.cpu().tolist())
        projected_embeddings.extend(projected.cpu().tolist())
        LOGGER.info("Encoded %s/%s lyrics", min(start + batch_size, total), total)

    return e5_embeddings, projected_embeddings


def write_metadata(output_dir: Path, args: argparse.Namespace, row_count: int, device: torch.device) -> None:
    metadata = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preprocess all music_lyrics into local multilingual-e5-large embeddings."
    )
...
def main() -> None:
    ...
    LOGGER.info("Using batch size: %s", args.batch_size)
    LOGGER.info("Loading lyrics from: %s", args.raw_dataset)
    df = load_lyrics(args.raw_dataset, args.limit)
    LOGGER.info("Loaded %s lyrics", df.height)
    ...
    output = df.select(["music_id", "music_name", "music_lang", "clean_lyrics"]).with_columns(
        pl.Series("lyrics_e5_large_embedding", e5_embeddings),
    ...
    write_metadata(args.output_dir, args, output.height, device)

    LOGGER.info("Wrote %s rows to %s", output.height, output_path)
    LOGGER.info("Wrote projection weights to %s", projection_path)
