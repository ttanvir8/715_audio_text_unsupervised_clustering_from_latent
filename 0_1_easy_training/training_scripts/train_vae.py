...
def log(message: str) -> None:
    print(f"[train_lyrics_vae] {message}", flush=True)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a basic lyrics-only VAE.")
    parser.add_argument(
        "--experiment-config",
        required=True,
...
def format_epoch_metrics(metrics: dict[str, float]) -> str:
    return (
        ...
        f"cosine={metrics.get('lyrics_cosine_loss', float('nan')):.6f}, "
...
def main() -> None:
    ...
    log(f"Lyrics parquet: {lyrics_path}")
    log(f"Input embedding column: {input_embedding_column}")
    log(f"Language split column: {language_column}")

    log("Loading row-aligned lyrics labels")
    ...
    include_clean_lyrics = bool(data_config.get("include_clean_lyrics", False))
    log("Building train dataset")
    train_dataset = LyricsVAEDataset(
    ...
    configured_input_dim = model_config.get("input_dim", "auto")
    if configured_input_dim in {None, "auto"}:
        model_config["input_dim"] = input_dim
    elif int(configured_input_dim) != input_dim:
        raise ValueError(
            f"Config model.input_dim={configured_input_dim} does not match "
            f"{input_embedding_column} dim={input_dim}."
        )
    ...
    log("Building BasicLyricsVAE model")
