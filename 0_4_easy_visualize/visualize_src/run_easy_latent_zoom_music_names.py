...
def log(message: str) -> None:
    print(f"[easy_latent_zoom_names] {message}", flush=True)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create zoomed latent-space plots with music_name labels on latent points."
    )
    parser.add_argument(
        "--points-root",
        default="0_4_easy_visualize/visualization_outputs/lyrics_vae_run_001",
        help="Root containing scope/letent_space point tables.",
    )
    parser.add_argument(
        "--output-root",
        default="0_4_easy_visualize/visualization_outputs/lyrics_vae_run_001",
...
def load_points(points_root: Path, scope: str, method: str, embedding: str) -> pd.DataFrame:
    path = points_root / scope / "letent_space" / f"{method}_{embedding}_points.parquet"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing point table: {path}. Run run_easy_latent_space.py first."
        )
...
def axis_limits(focus: pd.DataFrame, x_col: str, y_col: str, padding_fraction: float) -> tuple[tuple[float, float], tuple[float, float]]:
...
def plot_zoom(
    ...
    ax.set_ylim(*ylim)
    ax.set_title(
        f"{scope} {method.upper()} {embedding} zoom: {focus_name.replace('_', ' ')}"
    )
...
def run_scope(args: argparse.Namespace, scope: str) -> dict[str, Any]:
    ...
    output_dir = output_root / scope / "letent_space" / "zoomed_music_names"
    ...
    log(
        f"Zooming {scope}: method={args.method}, embedding={args.embedding}, "
        f"focus_rows={len(focus)}, labels={len(label_rows)}, focus={focus_name}"
    )
    plot_zoom(
        points=points,
        focus=focus,
    ...
    manifest = {
        ...
        "method": args.method,
        "embedding": args.embedding,
        "points_path": str(points_root / scope / "letent_space" / f"{args.method}_{args.embedding}_points.parquet"),
        ...
        "seed": int(args.seed),
        "artifacts": [str(plot_path), str(labels_path), str(manifest_path)],
    }
...
def main() -> None:
    ...
    save_json(
        ...
        / "letent_space"
        / f"{args.method}_{args.embedding}_zoomed_music_names_manifest.json",
    )
