...
def log(message: str) -> None:
    print(f"[train_vae] {message}", flush=True)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the MM-CBetaVAE.")
    parser.add_argument(
        "--experiment-config",
        required=True,
...
def make_run_dirs(output_dir: Path) -> dict[str, Path]:
    paths = {
        ...
        "reports": output_dir / "clustering_reports",
    }
        ...
        if step_log_every > 0 and (
            ...
            step_metrics.append(
                step_metric_row(
                    ...
                    global_step=global_step,
                    batch_in_epoch=batch_in_epoch,
                    batch_size=int(prepared["mel_norm"].shape[0]),
...
def main() -> None:
    ...
    log(
        ...
        f"device={training_config.get('device', 'cuda')}"
    )
    ...
    log(
        ...
        f"stratify_by_used={split['stratify_by_used']}"
    )

    batch_size = int(training_config.get("batch_size", 8))
    num_workers = int(data_config.get("num_workers", 0))
    log(f"Selecting device from setting: {training_config.get('device', 'cuda')}")
    device = select_device(str(training_config.get("device", "cuda")))
    ...
    log("Building MM-CBetaVAE model")
    ...
    mu, assisted, row_indices = export_latents(
        model,
        val_loader,
        normalizer,
        device,
    ...
    np.save(run_paths["latents"] / "val_metadata_assisted.npy", assisted)
    log(
        "Saved latent arrays: "
        f"val_mu={mu.shape}, val_metadata_assisted={assisted.shape}"
    )
    ...
    save_json(
        {
            ...
            "val_metadata_assisted_shape": list(assisted.shape),
