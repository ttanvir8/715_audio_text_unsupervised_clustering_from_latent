...
def compute_vae_loss(
    ...
    lyrics_cosine = 1.0 - F.cosine_similarity(lyrics_recon_unit, lyrics_target_unit, dim=-1).mean()
    lyrics_mse = F.mse_loss(outputs["lyrics_recon"], targets["lyrics"])
    lyrics_recon_loss = lyrics_cosine + 0.05 * lyrics_mse

    metadata_recon_loss = F.mse_loss(outputs["metadata_recon"], targets["metadata"])
    recon_loss = (
        config.mel_weight * mel_recon_loss
        + config.lyrics_weight * lyrics_recon_loss
        + config.metadata_weight * metadata_recon_loss
    )
    ...
    metrics = {
        ...
        "metadata_recon_loss": float(metadata_recon_loss.detach().cpu()),
...
def build_loss_config(config_dict: dict) -> LossConfig:
    return LossConfig(
        ...
        metadata_weight=float(config_dict.get("metadata_weight", 0.2)),
        beta_max=float(config_dict.get("beta_max", 4.0)),
