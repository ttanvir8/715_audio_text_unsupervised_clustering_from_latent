...
def prepare_batch(
    ...
    lyrics = batch["lyrics_e5_large_embedding"].to(device, non_blocking=True)
    main_genre = batch["main_genre_embedding"].to(device, non_blocking=True)
    music_lang = batch["music_lang_embedding"].to(device, non_blocking=True)
    condition = batch["condition_embedding"].to(device, non_blocking=True)
    metadata = torch.cat([main_genre, music_lang], dim=-1)
