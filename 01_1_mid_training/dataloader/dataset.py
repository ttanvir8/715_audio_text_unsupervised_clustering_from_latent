...
class AudioLyricsVAEDataset(Dataset):
    """Loads row-aligned lyrics embeddings and mel spectrogram tensors."""
    ...
    def __init__(
        ...
        columns = [*BASE_MODEL_INPUT_COLUMNS, input_embedding_column]
        self.features = pq.read_table(self.model_input_path, columns=columns)
        ...
        if self.length == 0:
            raise ValueError("AudioLyricsVAEDataset received no rows.")

        self._mel_cache: OrderedDict[int, torch.Tensor] = OrderedDict()
        first_row = self.indices[0]
        self.input_dim = len(self._feature_value(input_embedding_column, first_row))
        self.mel_shape = tuple(int(value) for value in self._mel_value(first_row).shape)
...
def audio_lyrics_collate_fn(batch: list[dict[str, Any]]) -> dict[str, Any]:
    output: dict[str, Any] = {
        "lyrics_input": torch.stack([item["lyrics_input"] for item in batch], dim=0),
        "melspectrogram": torch.stack([item["melspectrogram"] for item in batch], dim=0),
        ...
        "art_id": [item["art_id"] for item in batch],
        "art_name": [item["art_name"] for item in batch],
    }
    return output
